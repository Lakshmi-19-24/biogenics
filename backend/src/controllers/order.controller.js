...
export const deleteOrder = asyncHandler(async (req, res) => {
  ...
  const stockRestores = (order.items || [])
    .filter((item) => item.product && Number(item.allocatedQuantity || 0) > 0)
    .map((item) => ({
      updateOne: {
        filter: { _id: item.product },
        update: { $inc: { stock: Number(item.allocatedQuantity || 0) } }
      }
    }));

  if (stockRestores.length) {
    await Product.bulkWrite(stockRestores);
  }

  await Promise.all([
    InventoryMovement.deleteMany({ referenceType: 'Order', referenceId: order._id }),
    Payment.deleteMany({ order: order._id }),
    Invoice.deleteMany({ order: order._id }),
    Document.deleteMany({ order: order._id })
  ]);
  ...
  await notifyRoles(MANAGEMENT_ROLES, {
    title: 'Order deleted',
    message: `${req.user.name} deleted ${order.orderNo}.`,
    type: 'order',
    data: {
  ...
  sendResponse(res, 200, 'Order deleted', { id: order._id, orderNo: order.orderNo });
});

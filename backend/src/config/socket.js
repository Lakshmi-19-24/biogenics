...
export const initSocket = (server) => {
  io = new Server(server, {
    cors: {
      origin: (origin, callback) => {
        ...
        console.warn(
          `Socket CORS blocked for origin: ${origin}`
        );

        return callback(
          new Error(`Socket CORS blocked for origin: ${origin}`)
        );
      },
  ...
  io.use((socket, next) => {
    try {
      ...
      if (!token) {
        return next(
          new Error('Socket auth token is required')
        );
      }
      ...
      async (payload, ack) => {
        ...
        try {
          if (
            ...
            return fail(
              'Only sales representatives can report location'
            );
          }
          ...
          if (
            ...
            return fail(
              'Location accuracy too low to record'
            );
          }
          ...
          if (
            ...
            return fail(
              'Location tracking is only active between check-in and check-out'
            );
          }
          ...
          /*
           * Only OWNER and MANAGER receive
           * live sales location updates.
           *

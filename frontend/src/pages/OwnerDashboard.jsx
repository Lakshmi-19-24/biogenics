...
import { DollarSign, ShoppingCart, Users, UserCheck, TrendingUp, Target, ArrowUpRight, Crown, Plus, UserPlus, BarChart3 } from "lucide-react";
...
export default function OwnerDashboard() {
  ...
  const orderCols = [
    ...
    { header:"Order",    accessor:"orderId",     render:r=><span className="font-mono" style={{ fontSize:"12px",fontWeight:600,background:"rgba(124,58,237,0.08)",color:"#7c3aed",padding:"3px 8px",borderRadius:"6px" }}>#{r.orderId||r._id?.slice(-6)||"—"}</span> },
  ...
  return (
    ...
    <div className="page-enter">
      ...
      <PageHeader eyebrow="Owner" title="Dashboard" subtitle={today} action={

        <div style={{ display:"flex",alignItems:"center",gap:"8px",padding:"8px 14px",background:"rgba(124,58,237,0.08)",border:"1px solid rgba(124,58,237,0.22)",borderRadius:"10px" }}>

          <Crown size={14} style={{ color:"#7c3aed" }}/>

          <span style={{ fontSize:"13px",fontWeight:600,color:"#7c3aed" }}>Owner View</span>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"16px",marginBottom:"24px" }} className="stagger">
        ...
        <StatsCard icon={DollarSign} label="Total Revenue"  value={stats?.revenue||"₹0"} trend={stats?.revenueTrend} trendLabel="vs last month" color="success"/>

        <StatsCard icon={ShoppingCart} label="Total Orders" value={stats?.orders||"0"}   trend={stats?.ordersTrend} trendLabel="vs last month" color="info"/>

        <StatsCard icon={Users} label="Active Leads"        value={stats?.leads||"0"}    trend={stats?.leadsTrend}  trendLabel="vs last month" color="warning"/>

        <StatsCard icon={UserCheck} label="Team Members"    value={stats?.teamSize||"0"} color="primary"/>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"minmax(0,3fr) minmax(320px,2fr)",gap:"16px",marginBottom:"18px" }}>
        ...
        <div className="card" style={{ padding:"20px" }}>
          ...
          <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"20px" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:"6px",padding:"5px 10px",background:"rgba(124,58,237,0.08)",borderRadius:"8px" }}>

              <ArrowUpRight size={13} style={{ color:"#7c3aed" }}/>

              <span style={{ fontSize:"12px",fontWeight:700,color:"#7c3aed" }}>+12%</span>
            ...
            <ResponsiveContainer width="100%" height={250}>
              ...
              <AreaChart data={chartData} margin={{top:4,right:8,bottom:0,left:-12}}>
                ...
                <Area type="monotone" dataKey="revenue" stroke="#3B82F6" fill="url(#ownerRg)" strokeWidth={2.5} dot={{r:3,fill:"#60A5FA",strokeWidth:0}} activeDot={{r:6}}/>
      ...
      <div className="card" style={{ padding:"24px" }}>
        ...
        <p style={{ fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:"16px",fontWeight:700,color:"var(--text)",letterSpacing:"-0.03em",marginBottom:"16px" }}>Recent Orders</p>

        <DataTable columns={orderCols} data={recentOrders} pageSize={5} emptyTitle="No orders yet" emptyMessage="Orders will appear here once created." emptyAction={() => navigate("/orders")} emptyActionLabel="Create First Order"/>
              ...
              <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
                ...
                {row.role === 'sales' && (
                  <button
                    ...
                    style={{ padding:"6px 12px", fontSize:"12px", borderColor:"rgba(239,68,68,0.35)", color:"#f87171" }}
                  >
                    <Trash2 size={14} /> {deletingUserId === row._id ? 'Deleting…' : 'Delete'}
                  </button>
                )}
              </div>
            )}
